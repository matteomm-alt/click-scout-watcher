...
type LineupSnapshot = Pick<MatchState, 'homeCurrentLineup' | 'awayCurrentLineup' | 'homeSetterPosition' | 'awaySetterPosition' | 'servingTeam'>;
  ...
  persist(
    (set, get) => ({
      ...
      undoLastAction: () => set((s) => {
        const snapshot = lineupSnapshots.pop();
        return {
          matchState: {
            ...s.matchState,
            ...(snapshot ? {
